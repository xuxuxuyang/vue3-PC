...
#### 2. npm install normalize.css 做一下css初始化

## 一. 代码规范

### 1.1. 集成editorconfig配置（根目录下创建.editorconfig文件 复制下面内容到文件中+安装插件 EditorConfig for VS Code才会生效）
...
1.安装prettier （Development开发环境下使用）

```shell
npm install prettier -D 
```

2.配置.prettierrc文件：（根目录下创建.editorconfig文件 复制下面内容到文件中）
...
3.创建.prettierignore忽略文件（根目录下创建.editorconfig文件 复制下面内容到文件中）
...
4.VSCode需要安装Prettier - Code formatter的插件
...
* 测试一：在代码中保存代码；
* 测试二：配置一次性修改的命令；（配置完 使用 npm run prettier可执行）
...
在.eslintrc.js文件中（创建项目选择了eslint会自动创建） 添加prettier插件（在原来的配置中加最后一项即可）：

'plugin:prettier/recommended'    （添加完关闭项目再打开即可生效） 不用TS就不用写TS的
...
* 那么我们需要在组员执行 `git commit-m '...' ` 命令的时候对其进行校验，如果不符合eslint规范，那么自动通过规范进行修复；
...
npx husky-init  '&&' npm install (windows不行 加个引号即可) 然后去.husky文件里的pre-commit文件里

把npm test 改成npm run lint 即可再提交格式错误代码时自动格式化修复
...
接下来，我们需要去完成一个操作：为了在进行commit时，执行lint脚本：
...
* 第六步是否影响某个open issue（开源项目别人提交的issue）
...
手动在commit-msg文件里第四行加上 npx --no-install commitlint --edit 

## 二. 第三方库集成

### 2.1. vue.config.js配置（在根目录下创建）
...
// elecomps(app)
export function elecomps(app) {  //app是main.js中传过来的注册的app实例
...
### 2.6. VSCode配置（文件->首选项->设置->输入setting->点击在setting.json中编辑 复制即可）
